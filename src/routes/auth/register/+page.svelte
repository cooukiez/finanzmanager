...
<div class="m-2 pt-20 flex flex-row justify-center items-center">
  <Card.Root class="w-96">
    <Card.Header>
      ...
      <!-- Titel des Formulars: Registrierung -->
      <Card.Description>Create new account</Card.Description>
      <!-- Beschreibung des Formulars -->
    </Card.Header>
    <Card.Content>
      <form method="POST" use:enhance>
        <!-- Das Formular wird mit POST übermittelt und mit Enhance optimiert -->
        <!-- E-Mail-Feld -->
        <Form.Field {form} name="email">
          <Form.Control>
            {#snippet children({ props })}
              <Form.Label>Email</Form.Label>
              <!-- Label für das E-Mail-Feld -->
              <Input {...props} bind:value={$formData.email} />
              <!-- Bindet das Eingabefeld an die E-Mail-Daten -->
            {/snippet}
          </Form.Control>
          <Form.FieldErrors />
          <!-- Zeigt Fehler für das E-Mail-Feld an -->
        </Form.Field>
        ...
        <Form.Field {form} name="username">
          <Form.Control>
            {#snippet children({ props })}
              ...
              <!-- Label für das Benutzernamen-Feld -->
              <Input {...props} bind:value={$formData.username} />
              <!-- Bindet das Eingabefeld an die Benutzernamen-Daten -->
            {/snippet}
          </Form.Control>
          <Form.FieldErrors />
          <!-- Zeigt Fehler für das Benutzernamen-Feld an -->
        </Form.Field>
        ...
        <Form.Field {form} name="password">
          ...
          <Form.FieldErrors />
          <!-- Zeigt Fehler für das Passwort-Feld an -->
        </Form.Field>
